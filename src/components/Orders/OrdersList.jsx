...
export const OrdersList = ({ setOpenOrderDetails, updateOrderCounts }) => {
    ...
    const fetchFilteredOrders = async () => {
      ...
      try {
        ...
        if (response.data.orders.length === 0) {
          toast.warning("No orders found for this filter");
        }
      } catch (error) {
  ...
  return (
    <div className="lg:bg-white xl:p-4  rounded-lg flex-grow">
        ...
        <div className="my-4">
          <div className="hidden md:block">
            <div className="flex items-center justify-between border-b-2 text-left font-medium text-sm text-gray-400 p-1">
              <h6 className="w-1/6 py-1">Order ID</h6>
              <h6 className="w-1/6 py-1">Customer</h6>
              <h6 className="w-1/6 py-1">Order Value</h6>
              <h6 className="w-1/6 py-1">Ordering Date</h6>
              <h6 className="w-1/6 py-1">Status</h6>
            ...
            {displayedOrders.map((order) => (
              <div
                key={order._id}
                className="flex items-center justify-between border-b-2 text-left font-medium p-1 hover:bg-gray-50 transition duration-300"
              >
                <p className="w-1/6 py-1">{order.orderId}</p>
                <p className="w-1/6 py-1">{order.userId?.name}</p>
                <p className="w-1/6 py-1">₹{order.finalBillToPay.toFixed(2)}</p>
                <p className="w-1/6 py-1">
                  {moment(order.createdAt).format("DD MMM YYYY, h:mm A")}
                </p>
                <div className="w-1/6 py-1 relative">
                  <div
          ...
          <div className="md:hidden">
            {displayedOrders.map((order) => (
              <div
                key={order._id}
                className="bg-white rounded-lg shadow-md p-4 mb-2 hover:shadow-lg transition duration-300"
              >
                ...
                <div className="mt-2 space-y-0">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium">₹{order.finalBillToPay.toFixed(2)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-600">Ordering Date:</span>
                  ...
                  <div className="flex justify-between">
                    ...
                    <span
                      className={`font-bold ${getStatusColor(orderStatuses[order._id])}`}
                    >
                      {orderStatuses[order._id]}
                    </span>
                  </div>
